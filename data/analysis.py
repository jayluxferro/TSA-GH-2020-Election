...
df = full_data[["Tweet Text"]]
df["text"] = df["Tweet Text"].astype(str)
full_data.head(10).to_latex("sample_tweets.tex")
full_data.head(10)
...
def convert_emoticons(txt):
    for emot in EMOTICONS:
        txt = re.sub(u'('+emot+')', "_".join(EMOTICONS[emot].replace(",","").split()), txt)
    return txt
...
def clean_text(txt):
    txt = re.sub(r'@[A-Za-z0-9]+', '', txt)  # remove the @ delimiter
    txt = re.sub(r'#', '', txt)  # remove all hash tags
    txt = re.sub(r'//', '', txt)  # remove all hash forward slashes
    txt = re.sub(r'\\', '', txt)  # remove all hash backward slashes
    txt = re.sub(r'rt[\s]+', '', txt)  # remove the text RT for re-tweets
    txt = re.sub(r':[\s]+', '', txt)  # remove the text : (colons)
    txt = re.sub(r'https?:\S+', '', txt)  # remove all urls

    return txt  # return the clean data
...
def remove_non_english(txt):
    " ".join(w for w in nltk.wordpunct_tokenize(txt)              if w.lower() in words or not w.isalpha())
...
# # # create a column for text to process
df['Pro-Text'] = df['text'].astype(str)
# # # convert text to lower case
df['Pro-Text'] = df['text'].str.lower()
df.head(10).to_latex("process_1.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(clean_text)
df['Pro-Text'] = df['Pro-Text'].apply(remove_urls)
df.head(10).to_latex("process_2.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(remove_emoticons)
df.head(10).to_latex("process_3.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(convert_emoticons)
df.head(10).to_latex("process_4.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(stem_words)
df.head(10).to_latex("process_5.tex")
df
...
df["Pro-Text"] = df["Pro-Text"].apply(lambda txt: lemmatize_words(txt))
df.head(10).to_latex("process_6.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(lemmatize_words)
df.head(10).to_latex("process_7.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(remove_emoji)
df.head(10).to_latex("process_8.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(remove_punctuation)
df.head(10).to_latex("process_9.tex")
df
...
df['Pro-Text'] = df['Pro-Text'].apply(remove_stopwords)
df.head(10).to_latex("process_10.tex")
df
...
df['Sentiment'] = df['Pro-Text'].apply(lambda x: TextBlob(x).sentiment)
df.head(10).to_latex("process_11.tex")
df
...
df['Polarity'] = df['Pro-Text'].apply(lambda x: TextBlob(x).sentiment.polarity)
df.head(10).to_latex("process_12.tex")
df
...
df['Subjectivity'] = df['Pro-Text'].apply(lambda x: TextBlob(x).sentiment[1])
...
def classifier(polar):
    if polar > 0:
        ...
        return 'negative'
df['Polarity'] = df['Pro-Text'].apply(lambda x: TextBlob(x).sentiment.polarity)
df.head(10).to_latex("process_13.tex")
df
...
df['Subjectivity'] = df['Polarity'].apply(classifier)
tweets = df["Pro-Text"]
subjectivity = df["Subjectivity"].astype(str)
df.head(10).to_latex("process_14.tex")
df
...
for tweet,polarity in zip(tweets, subjectivity):
    npp = ["npp", "nana", "4morefornana", "bawumia", "leadershipofservice"]
    ndc = ["ndc", "mahama", "JohnMahama2020", "Opoku-Agyemang", "ndcmanifesto"]
    words = tweet.split()
    # for the NPP party
    if ((npp[0] in words) or (npp[1] in words) or (npp[2] in words) or (npp[3] in words) or (npp[4] in words)) and (polarity == "positive"):
        npp_positive += 1
    elif ((npp[0] in words) or (npp[1] in words) or (npp[2] in words) or (npp[3] in words) or (npp[4] in words))  and (polarity == "negative"):
        npp_negative += 1
    elif ((npp[0] in words) or (npp[1] in words) or (npp[2] in words) or (npp[3] in words) or (npp[4] in words))  and (polarity == "neutral"):
        npp_neutral += 1
    # NDC party
    elif ((ndc[0] in words) or (ndc[1] in words) or (ndc[2] in words) or (ndc[3] in words) or (ndc[4] in words))  and (polarity == "positive"):
        ndc_positive += 1
    elif ((ndc[0] in words) or (ndc[1] in words) or (ndc[2] in words) or (ndc[3] in words) or (ndc[4] in words)) and (polarity == "negative"):
        ndc_negative += 1
    elif ((ndc[0] in words) or (ndc[1] in words) or (ndc[2] in words) or (ndc[3] in words) or (ndc[4] in words)) and (polarity == "neutral"):
        ndc_neutral += 1


print("Scores in Details: NPP \n")
print(f"NPP-positive : {npp_positive}")
print(f"NPP-negative : {npp_negative}")
print(f"NPP-neutral : {npp_neutral}")

print("\nScores in Details: NDC")
print(f"NDC-positive : {ndc_positive}")
print(f"NDC-negative : {ndc_negative}")
print(f"NDC-neutral : {ndc_neutral}")

total_neutral = npp_neutral + ndc_neutral
print("\n")
print(f"Total neutral : {total_neutral}")

ndc_total = ndc_positive + ndc_negative + ndc_neutral
npp_total = npp_positive + npp_negative + npp_neutral
total_tweets = float(ndc_total + npp_total)
print(f"Total Tweets Containing Keywords: {int(total_tweets)} Tweets")
...
# NPP Percentage
NppPositive = float((npp_positive / total_tweets) * 100)
NppNegative = float((npp_negative / total_tweets) * 100)
NppNeutral = float((npp_neutral / total_tweets) * 100)

NdcPositive = float((ndc_positive / total_tweets) * 100)
NdcNegative = float((ndc_negative / total_tweets) * 100)
NdcNeutral = float((ndc_neutral / total_tweets) * 100)
print("Scores in Percentage : \n")
print(f"NDC POSITIVE % : {NdcPositive}")
print(f"NDC NEGATIVE % : {NdcNegative}")
print(f"NDC NEUTRAL % : {NdcNeutral}")

print("\n")
print(f"NPP POSITIVE % : {NppPositive}")
print(f"NPP NEGATIVE % : {NppNegative}")
print(f"NPP NEUTRAL %  : {NppNeutral}")

print("\n")
total_percentage = NdcPositive + NdcNegative + NdcNeutral + NppPositive + NppNegative + NppNeutral
print(f"Total Percentage: {total_percentage}")


# In[ ]:
data = [
    NppPositive, NppNeutral, NppNegative,
    NdcPositive, NdcNeutral, NdcNegative
]

labels = [
    'NPP POSITIVE', 'NPP NEUTRAL' ,'NPP NEGATIVE',
    'NDC POSITIVE', 'NDC NEUTRAL' ,'NDC NEGATIVE'
]
...
plt.tight_layout()
plt.savefig('analysis_1.eps', dpi=1200)
#plt.show()


# In[ ]:
# # # Positive Against Positive
data = [
    NppPositive,
    NdcPositive
]
...
plt.title("Analysis of Tweets (Positives)\n")
plt.axis('equal')
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('analysis_2.eps', dpi=1200)
#plt.show()


# In[ ]:
width = 0.4
ticks = ['POSITIVE', 'NEGATIVE', 'NEUTRAL']
npp_bar = [NppPositive, NppNegative, NppNeutral]
ndc_bar = [NdcPositive, NdcNegative, NdcNeutral]

bar1 = np.arange(len(ticks))
bar2 = [i+width for i in bar1]

plt.figure()
plt.bar(bar1, npp_bar, width, label="NPP")
plt.bar(bar2, ndc_bar, width, label="NDC")
plt.xlabel("PARTIES")
plt.ylabel("SCORE")
plt.title("COMPLETE ANALYSIS")
plt.xticks(bar1 + (width/2), ticks)
plt.legend()
plt.savefig('analysis_3.eps', dpi=1200)
#plt.show()
